"""Build the ssl_behavior dataset: compressed continuous keypoint tracking (DLC-style
face/whisker/top-camera tracking: jaw, nose, tongue, pupil, spout, particle, whisker)
from Axel Bisi's single-session-learning (SSL) NWB files.

Source is the same local NWB directory as ssl_ephys, but this module only reads
processing/behavior/BehavioralTimeSeries. See ssl_ephys.py for the session/trial/
epoch/spike tables and the general dataset-contract conventions (schema.yaml,
provenance.yaml, parquet metadata) that this module mirrors.

Timestamp integrity is session-specific and NOT assumed: some sessions have no
BehavioralTimeSeries at all (no video), some have per-keypoint `data` and
`timestamps` arrays of equal length (valid), and some have `timestamps` that are
empty or a different length than `data` (invalid -- observed in real files, not
hypothetical). Only keypoints with data_len == timestamps_len > 0 are compressed;
everything else is recorded as unavailable in keypoint_availability.parquet and
skipped, rather than guessing a start time or sample rate.

Within one session, keypoints are further split into "timing groups": keypoints
whose (data_len, timestamps_len) match exactly are assumed to share one camera's
frame clock and get one shared, delta-encoded timestamps array; keypoints with a
different length get their own group. This was verified empirically -- keypoint
name prefixes (e.g. "top_") do not reliably predict which keypoints share timing.
"""
...
from ibl_ai_agent.datasets.ssl_ephys import SESSION_FILENAME_RE, _list_session_files


DATASET_NAME = "ssl_behavior"
...
def _read_one_session(path: Path, *, quantization_us: int, precision: float) -> dict[str, Any]:
    ...
    with NWBHDF5IO(str(path), "r", load_namespaces=True) as io:
        ...
        # The largest group by keypoint count is treated as the "primary" valid
        # timing group only if it has a non-zero, matching (data_len == timestamps_len);
        # every matching-length group is compressed, every mismatched one is skipped.
        group_index = 0
...
def load_tracking_shard(path: Path) -> dict[str, Any]:
    """Decode a ssl_behavior tracking shard: reconstruct real timestamps and values."""
...
def build_ssl_behavior_dataset(config: BuildConfig) -> BuildOutputs:

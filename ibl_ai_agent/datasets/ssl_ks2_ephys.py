"""Build the ssl_ks2_ephys dataset: a compressed local dataset from Axel Bisi's
single-session-learning (SSL) whisker/auditory NWB files, KS2/"combined"
spike-sorting source (M:\\analysis\\Axel_Bisi\\NWB_combined).

This mirrors ssl_ephys.py (same session-file naming convention, same on-disk
contract: schema.yaml, provenance.yaml, parquet metadata tables, delta-encoded
spike shards), pointed at a different, largely-overlapping-but-distinct source
directory. Verified schema differences from the ssl_ephys/NWB_ks4 source
(session AB080_20230622_152205, AB116_20240724_102941 compared directly):

- Different unit counts and QC columns for the same physical sessions (e.g.
  892 vs 939 units for AB080_20230622_152205) -- an older/different
  spike-sorting or QC pipeline, not a re-export of the same sort. Units table
  is missing Lratio/isolationDistance/maxDriftEstimate/cumDriftEstimate
  (present in the ks4 source) and adds bc_cluster_id (not in the ks4 source).
  The generic numeric-coercion-by-exclusion-list approach below handles this
  schema difference without changes.
- cluster_id is still a string column, and is still only unique per probe
  (not per session) -- same fixes as ssl_ephys.py apply unchanged.
- No processing/behavior/BehavioralEpochs interface anywhere checked, even for
  post-AB116 sessions where the ssl_ephys/ks4 source has one. The trial-level
  `context` column ('passive'/'active') is present and used instead: epochs
  are derived from context + trial order (the same method validated in the
  ssl-passive-sensory-selectivity project), not read from BehavioralEpochs.
"""
...
# See module docstring: this source's units table has a different column set
# than ssl_ephys/NWB_ks4's. This exclusion list only names genuinely
# categorical/string columns; everything else is coerced to numeric
# regardless of which QC columns happen to be present in a given source.
UNIT_STRING_COLUMNS = {
...
def _extract_epoch_rows_from_context(session_id: str, trials_df: pd.DataFrame) -> list[dict[str, Any]]:
    """Derive passive_pre/active/passive_post epoch spans from the trial-level
    `context` column and trial order (this source has no BehavioralEpochs
    interface to read boundaries from directly). Method validated in the
    ssl-passive-sensory-selectivity project: passive_pre = context=='passive'
    before the session's first active trial; passive_post = context=='passive'
    after the last active trial; span bounds come from the actual covered
    trials' start/stop times, not fabricated boundaries.
    """
    if trials_df.empty or "context" not in trials_df.columns:
        return []
    t = trials_df.sort_values("start_time")
    active = t[t["context"] == "active"]
    passive = t[t["context"] == "passive"]
    rows: list[dict[str, Any]] = []
    if len(active):
        rows.append({
            "session_id": session_id, "epoch_name": "active",
            "start_time": float(active["start_time"].min()), "stop_time": float(active["stop_time"].max()),
        })
    if len(active) and len(passive):
        ...
        if len(passive_pre):
            rows.append({
                "session_id": session_id, "epoch_name": "passive_pre",
                "start_time": float(passive_pre["start_time"].min()), "stop_time": float(passive_pre["stop_time"].max()),
            })
        if len(passive_post):
            rows.append({
                "session_id": session_id, "epoch_name": "passive_post",
                "start_time": float(passive_post["start_time"].min()), "stop_time": float(passive_post["stop_time"].max()),
            })
    elif len(passive) and not len(active):
        rows.append({
            "session_id": session_id, "epoch_name": "passive",
            "start_time": float(passive["start_time"].min()), "stop_time": float(passive["stop_time"].max()),
        })
    return rows
...
def _read_one_session(path: Path, *, subject: str, date: str, time_str: str) -> dict[str, Any]:
    ...
    with NWBHDF5IO(str(path), "r", load_namespaces=True) as io:
        ...
        trials_df_raw = pd.DataFrame()
        if nwb.trials is not None and len(nwb.trials) > 0:
            trials_df_raw = nwb.trials.to_dataframe().reset_index(drop=True)

        epoch_rows = _extract_epoch_rows_from_context(session_id, trials_df_raw)

        trials_df = trials_df_raw.copy()
        if len(trials_df):
            trials_df.insert(0, "session_id", session_id)

        event_rows: list[dict[str, Any]] = []
        beh = nwb.processing.get("behavior") if nwb.processing else None
        if beh is not None and "BehavioralEvents" in beh.data_interfaces:
...
def load_spike_shard(path: Path) -> dict[str, Any]:
    """Decode a ssl_ks2_ephys spike shard, same contract as bwm_ephys.load_spike_shard."""
...
def build_ssl_ks2_ephys_dataset(config: BuildConfig) -> BuildOutputs:
    ...
    try:
        ...
        provenance = {
            ...
            "source": {
                ...
                "note": "KS2/combined spike-sorting source (NWB_combined), distinct from ssl_ephys's NWB_ks4 source. Epochs derived from the context column + trial order; no BehavioralEpochs interface present in this source.",
            },
            "spike_encoding": {
                "cluster_encoding": "dense_local_indices",
